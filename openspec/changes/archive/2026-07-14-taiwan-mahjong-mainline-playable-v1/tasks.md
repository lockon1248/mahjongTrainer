...
- [x] 5.1 建立 `taiwan-mahjong-human-self-turn-actions`：補齊真人自己回合可執行的動作入口。
- [x] 5.2 納入自摸胡、暗槓、加槓與補牌流程的 core / store / UI 邊界。
- [x] 5.3 驗證真人在自己回合不只可打牌，也能完成完整合法操作。
- [x] 5.4 這一步完成後，主線才可宣稱真人端具備基本可玩操作閉環。

## 6. 已完成：流局與莊家流程

- [x] 6.1 建立 `taiwan-mahjong-draw-outcome-and-dealer-flow`：補齊流局結果、莊家處理、下一局銜接。
- [x] 6.2 依既有 rules baseline 已定案內容實作；未定案部分不得猜測。
- [x] 6.3 驗證牌牆耗盡或流局條件成立時，牌局能正確結束或進入下一局。
- [x] 6.4 這一步完成後，主 spec `階段 3` 的 `流局／莊家處理` 才能標記為主線完成。

## 7. 已完成：結算結果接回 UI

- [x] 7.1 建立 `taiwan-mahjong-ui-round-result-sync`：讓胡牌 / 流局結果在牌桌 UI 完整反映。
- [x] 7.2 顯示主線必要資訊：胡牌者、放槍者、結果摘要、是否結束本局。
- [x] 7.3 驗證 UI 只做狀態映射，不在 view 層重算規則或結算。
- [x] 7.4 這一步完成後，主線才能視為從 core 結果成功接回玩家可見畫面。

## 8. 已完成：AI 主線補強

- [x] 8.1 建立 `taiwan-mahjong-ai-claim-quality-pass`：補齊 AI 在宣告與自回合的主線缺口。
- [x] 8.2 確保 AI 至少具備主線可玩所需的吃、碰、槓、胡決策，不要求進階最佳化。
- [x] 8.3 驗證 AI 不會因新增真人互動而失去既有自動推進能力。
- [x] 8.4 這一步完成後，主 spec 的 `1 位真人玩家 + 3 位 AI` 才算主線閉環成立。

## 9. 已完成：主線回歸驗證

- [x] 9.1 建立一組主線整合測試，覆蓋開局、摸打、宣告、胡牌或流局、結果同步。
- [x] 9.2 執行 typecheck、UI 測試、core 測試與 Spectra 分析，確認主線 change 與子 change 一致。
- [x] 9.3 任一子 change 完成後，立即回來更新本清單，打勾已完成項目並開始下一個主線任務。
- [x] 9.4 這一步完成後，主 spec 的「可完整進行基本牌局流程」才可在主線層級標記為完成。
